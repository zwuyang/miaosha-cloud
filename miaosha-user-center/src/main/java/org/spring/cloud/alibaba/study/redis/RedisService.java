...
public class RedisService {
    ...
    public <T> boolean set(KeyPrefix prefix, String key, List<T> list) {
        Jedis jedis = null;
        try {
            jedis = jedisPool.getResource();
            String realKey = createRealKey(prefix, key);
            String listStr = beanToString(list);
            Long res = jedis.rpush(realKey, listStr);
    ...
    public boolean setArr(KeyPrefix prefix, String key, String[] values) {
        if (values == null){
            return false;
        }
        Jedis jedis = null;
        try {
            jedis = jedisPool.getResource();
            String realKey = createRealKey(prefix, key);
            Long res = jedis.rpush(realKey, values);
            return res != null && res.intValue() > 0;
        } catch (Exception e) {
    ...
    public List<String> getArr(KeyPrefix keyPrefix, String key, int start, int end) {
        Jedis jedis = null;
        try {
            jedis = jedisPool.getResource();
            String realKey = createRealKey(keyPrefix, key);
            return jedis.lrange(realKey, start, end);
        } finally {
            closeResource(jedis);
        }
    }
    ...
    public Set<String> getSets(KeyPrefix keyPrefix, String key) {
        Jedis jedis = null;
        try {
            jedis = jedisPool.getResource();
            String realKey = createRealKey(keyPrefix, key);
    ...
    public boolean addSet(KeyPrefix prefix, String key, String[] values){
        Jedis jedis = null;
        try {
            jedis = jedisPool.getResource();
            String realKey = createRealKey(prefix, key);
            Long res = jedis.sadd(realKey, values);
            return res != null && res.intValue() > 0 ;
        }finally {
            closeResource(jedis);
        }
    ...
    public boolean addSet(KeyPrefix prefix, String key, String value){
        Jedis jedis = null;
        try {
            jedis = jedisPool.getResource();
            String realKey = createRealKey(prefix, key);
            Long res = jedis.sadd(realKey, value);
            return res != null && res.intValue() > 0 ;
        }finally {
            closeResource(jedis);
        }
    ...
    public <T> boolean existInSet(KeyPrefix prefix, String key,T value){
        Jedis jedis = null;
        try {
            jedis = jedisPool.getResource();
            String realKey = createRealKey(prefix, key);
            String beanStr = beanToString(value);
            return jedis.sismember(realKey, beanStr);
        }finally {
            closeResource(jedis);
        }
    }
